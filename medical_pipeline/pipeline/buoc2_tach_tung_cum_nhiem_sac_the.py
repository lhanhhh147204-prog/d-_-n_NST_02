...
class ChromosomeClusterSegmenter:
    ...
    def __init__(self, image, min_area=150):
        self.image = image
    ...
    def segment_clusters(self):
        ...
        for contour in contours:
            ...
            # Loại bỏ các hạt nhân tế bào hình tròn/bầu dục đặc hoặc cụm rác quá lớn
            is_nucleus = (solidity > 0.9 and 0.5 < aspect_ratio < 2.0)
            is_giant_blob = (solidity > 0.95 and area > 10000)
            # Loại bỏ dãy số, text hoặc vệt bẩn quá dài/quá dẹt
            is_watermark_text = (solidity > 0.8 and (aspect_ratio > 4.0 or aspect_ratio < 0.25))
            
            if is_nucleus or is_giant_blob or is_watermark_text:
        ...
        for idx, item in enumerate(valid_items):
            ...
            # Đánh số lên cụm NST nhận diện được
            text = f"#{idx}"
            cv2.putText(visualization_img, text, (x, max(15, y - 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
...
def stack_images(images, cols=5, bg_color=(255, 255, 255)):
    ...
    for idx, img in enumerate(images):
        ...
        # Đánh số lên góc trái trên của từng cell
        cv2.putText(stacked, f"#{idx}", (x + 5, y + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        
    return stacked


def run_segmentation(img_bgr):
    segmenter = ChromosomeClusterSegmenter(image=img_bgr, min_area=150)

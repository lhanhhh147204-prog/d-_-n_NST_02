...
def run_overlap_separation(input_dir: Path, output_dir: Path = None) -> list:
    ...
    classifier = ClusterClassifier.get_instance(weights_path)
    ...
    while queue:
        ...
        def is_valid_size(part_img):
            ...
            # Ngưỡng 150 đủ để giữ NST siêu nhỏ, trong khi buoc3_1 đã lo việc cấm cắt nát bằng balance ratio
            return np.sum(gray < 250) > 150
